...
async function modifySettings(projectPath: string) {
  try {
    ...
    if (vscodeFolder.includes('settings.json')) {
      ...
      settingsFile["json.schemas"].forEach(obj => removeWildcards(obj.fileMatch));
      const isConfigIncluded = settingsFile["json.schemas"].some(obj => obj.fileMatch.includes('msc.config.json'));
      if (!isConfigIncluded) {
