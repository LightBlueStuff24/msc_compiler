...
import Log, { FileUtils } from "@utils";
...
export async function Build() {
  const config: typeof configFile = await import(
    `${path.join(cwd, "msc.config.json")}`
  );
  if (!config) {
    Log.error(`Failed to find config in user directory`);
    process.exit(1);
  }

  const workspaceFiles = await FileUtils.getWorkspaceFiles();
...
async function BuildRegistryFiles() {
  fsPromise.mkdir("");
  //@ts-ignore
  const { BlockRegistry, ItemRegistry, EntityRegistry, AdvancementRegistry } = await import("./dist/index");
}

async function BuildRegistry(registry: any[], subFolder: string) {}
...
export async function Watcher(dir = cwd) {
