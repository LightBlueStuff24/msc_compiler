  ...
  static init() {
    ...
    if (this.Texture || this.RenderMethod || this.FaceDimming || this.AmbientOcclusion) {
      ...
      if (this.RenderMethod) {
        ...
        if (["opaque", "blend", "alpha_test", "double_sided"].includes(this.RenderMethod)) return new Error(`[${this.name}] [component: RenderMethod]: expected type {RenderMethod} but found {${this.RenderMethod}}`)
    ...
    if (this.Geometry || this.BoneVisibility) {
      ...
      if (this.BoneVisibility) {
        ...
        for (const [bone, value] of this.BoneVisibility) {
          if (typeof bone == "string" && ["string","boolean"].includes(typeof value)) {
          ...
          if (!["string","boolean"].includes(typeof value)) return new Error(`[${this.name}] [component: bone_visibility] [child: ${value}]: expected {string or boolean} instead found {${typeof value}}`)
        }
      }
    }
    // TODO: From here
    ...
    if (this.SelectionBox) {
      ...
      if (this.OnPlayerDestroyed) {
        ...
        if (this.OnPlayerDestroyed.Target) {
          ...
          if (!["self", "other"].includes(this.OnPlayerDestroyed.Target)) return new Error(`[${this.name}] [component: OnPlayerDestroyed] [child: Target]: expected type {Targets} instead found {${this.OnPlayerDestroyed.Target}}`)
      ...
      if (this.QueuedTicking) {
        ...
        if (this.QueuedTicking.IntervalRange) {
          if (!Array.isArray(this.QueuedTicking.IntervalRange)) return new Error(`[${this.name}] [component: QueuedTicking] [child: IntervalRange]: expected type {number[]} instead found {${typeof this.QueuedTicking.Condition}}`)
        ...
        if (this.QueuedTicking.Target) {
          if (typeof this.QueuedTicking.Target == "string") {
            if (["self", "other"].includes(this.QueuedTicking.Target)) {
      ...
      if (this.RandomTicking) {
        ...
        if (this.RandomTicking.Target) {
          if (typeof this.RandomTicking.Target == "string") {
            if (["self", "other"].includes(this.RandomTicking.Target)) {
      ...
      if (this.States) {
        ...
        for (let [state, values] of Object.entries(this.States)) {
          if (typeof state == "string") {
            ...
            __States[stateName] = [];
            values.forEach(v => {
              if (["boolean", "number"].includes(typeof values)) {
