  ...
  static async init() {
    ...
    for (const [cdata, cvalue] of Object.entries(this).filter(([key, val]) => val !== undefined)) {
      switch (cdata) {
        ...
        case "Namespace": {
          if (typeof cvalue != "string") return ME(this, cvalue, [cdata], "string");
          this.__Data["minecraft:block"].description.identifier.split(":")[0].replace(config.prefix, cvalue)
        }; break;
        case "Identifier": {
          if (typeof cvalue != "string") return ME(this, cvalue, [cdata], "string");
          console.warn(this.__Data["minecraft:block"].description.identifier.split(":")[1].replace(this.name.toLowerCase(), cvalue))
